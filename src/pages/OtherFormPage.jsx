...
import cdcLogo from "../assets/Fin.jpg";

export default function OtherFormPage({ isEmbedded = false }) {
  ...
  const saveSignature = () => {
    ...
    } else alert("Please sign before saving.");
  };

  const handleSubmit = async (e) => {
    e?.preventDefault?.();
    if (!signature) return alert("Please provide your signature before submitting.");
    setLoading(true);
    try {
      const fileName = `signatures2/${formData.matricule}_${Date.now()}.png`;
      ...
      await addDoc(collection(db, "otherforms"), {
        ...formData,
        signatureURL: sigURL,
  ...
  return (
    <div className={`${isEmbedded ? "" : "min-h-screen bg-gray-200 flex flex-col items-center py-6 px-4"}`}>
        ...
        <div className="flex justify-end items-end mt-16">
          <div className="text-right">
            ...
            <div className="flex justify-end gap-2 mt-2">
              <button type="button" onClick={clearSignature} className="bg-blue-600 text-white px-3 py-1 text-sm rounded">
                Clear
              </button>
              <button type="button" onClick={saveSignature} className="bg-blue-600 text-white px-3 py-1 text-sm rounded">
                Save
              </button>
            </div>
      ...
      {/* ✅ SUBMIT BUTTON (always visible, like FormPage.jsx) */}
